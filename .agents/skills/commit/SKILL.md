...
1. Determine trailer fields before drafting the final message:
   ...
   - `AGENT_NAME`: Use the current agent name exactly as given by the active
     agent context. For OpenCode, use `OpenCode`.
   - `MODEL_VERSION`: Prefer the project-level agent configuration. First read
     `opencode.json` and use its `model` value when present. If absent, use
     another project-level agent config that explicitly names the active model.
     If project config does not provide a model, use the exact model version
     exposed by the runtime or higher-level agent configuration. Do not guess a
     model family; if the exact model version cannot be determined, stop and
     report the problem.
