...
Use the active agent name, the exact model version from project configuration
when available, and bracketed external code analysis tool names only when such
tools were actually called. If no external code analysis tool was called, omit
the bracketed tool fields.
