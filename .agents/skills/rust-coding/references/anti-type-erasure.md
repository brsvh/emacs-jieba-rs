...
`Box<dyn Trait>` (type erasure) introduces heap allocation and dynamic dispatch
overhead. When you have a single concrete type or can use generics, `impl Trait`
provides the same flexibility with zero overhead through monomorphization.
...
- [trait-dyn-vs-generic](./trait-dyn-vs-generic.md) - Choose dispatch
  deliberately
- [closure-static-vs-dyn](./closure-static-vs-dyn.md) - Same tradeoff for
  closures
