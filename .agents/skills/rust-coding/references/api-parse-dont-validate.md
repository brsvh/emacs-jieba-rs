...
Instead of validating data and hoping you remember to check everywhere, parse it
into a type that can only be constructed from valid data. The type system then
guarantees validity - you can't forget to validate because invalid states are
unrepresentable.
...
- [serde-try-from-validate](serde-try-from-validate.md) - Validate at the serde
  boundary
