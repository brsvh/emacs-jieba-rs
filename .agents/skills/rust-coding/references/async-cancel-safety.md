...
`tokio::select!` polls multiple futures concurrently and, the moment one branch
completes, it **drops every other branch** — including any state those futures
held. A future that was halfway through reading bytes into a local buffer, or
halfway through draining a channel into a `Vec`, loses that progress silently.
This is not a compiler error; it compiles fine and the bug only surfaces under
concurrent load. Tokio documents which of its primitives are cancellation-safe;
everything else must be treated with care.
...
1. **Move state outside `select!`** — keep the accumulation buffer or partial
...
- [async-select-racing](async-select-racing.md) - use `tokio::select!` for
  racing/timeouts
- [async-bounded-channel](async-bounded-channel.md) - use bounded channels for
  backpressure
- [async-no-lock-await](async-no-lock-await.md) - never hold locks across
  `.await`
