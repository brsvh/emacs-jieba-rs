...
`tokio::sync::mpsc` (multi-producer, single-consumer) is the workhorse channel
for async Rust. It provides async send/receive, backpressure via bounded
capacity, and efficient cloning of senders. It's the default choice for
...
- [async-oneshot-response](./async-oneshot-response.md) - Request-response with
  oneshot
