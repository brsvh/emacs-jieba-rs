...
Holding a lock across an `.await` point can cause deadlocks and severely hurt
performance. The task may be suspended while holding the lock, blocking all
other tasks waiting for it - potentially indefinitely.
...
- [async-spawn-blocking](async-spawn-blocking.md) - Use spawn_blocking for CPU
  work
