...
completes. In async code, this blocks the executor thread, preventing it from
running other tasks. `tokio::fs` wraps filesystem operations in
`spawn_blocking`, keeping the executor responsive.
...
- [async-spawn-blocking](./async-spawn-blocking.md) - How tokio::fs works
  internally
