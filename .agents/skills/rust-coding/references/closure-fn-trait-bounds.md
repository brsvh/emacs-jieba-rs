...
> Require the least restrictive `Fn` trait a callback needs (`FnOnce` ⊇ `FnMut`
> ⊇ `Fn`)

## Why It Matters

`FnOnce` is implemented by every closure — it may consume its captures and can
only be called once. `FnMut` is implemented by closures that mutate captures,
and implies `FnOnce`. `Fn` is the strictest: it only reads captures and can be
called any number of times concurrently. Bounding a parameter with the weakest
trait the body actually requires accepts the widest set of callers. Requiring
`Fn` when you only call the closure once needlessly rejects move-consuming
closures.
...
- `FnMut` requires `mut f` at the call site (the parameter or binding must be
  `mut`).
- `Fn: FnMut: FnOnce` — a `Fn` closure satisfies an `FnOnce` bound, not the
  other way around.
- Standard library examples: `Iterator::map` uses `FnMut`; `thread::spawn` uses
  `FnOnce + Send + 'static`.

## See Also

- [closure-static-vs-dyn](closure-static-vs-dyn.md) - generic vs dynamic
  dispatch for callbacks
- [closure-move-capture](closure-move-capture.md) - when and how to use `move`
  closures
