...
`impl Fn` in return position names the closure's concrete (but unnameable) type
and enables static dispatch with no heap allocation. `Box<dyn Fn>` adds an
allocation and a virtual call every time the closure is invoked. The opaque
`impl Trait` syntax was designed precisely for this use case. Reach for
`Box<dyn Fn>` only when the function must return *different* closure types
depending on runtime conditions, or when the closure must be stored in a struct
field or collection.
...
Different `if`/`match` arms return distinct closure types, so `impl Fn` cannot
unify them:
...
- [anti-type-erasure](anti-type-erasure.md) - avoid `Box<dyn Trait>` when
  `impl Trait` works
- [closure-static-vs-dyn](closure-static-vs-dyn.md) - static vs dynamic dispatch
  trade-offs for callbacks
