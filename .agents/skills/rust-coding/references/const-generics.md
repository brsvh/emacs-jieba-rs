...
Const generics let a single type or function work for any array size — or other
constant value — without macros, trait objects, or carrying a runtime length
field. The compiler monomorphizes one copy per distinct value, so there is no
indirection and no overhead compared to hand-writing the same code for each
size. This is the idiomatic way to write generic array-based data structures and
algorithms on stable Rust.
...
(`struct Buf<const N: usize = 64>`), letting you provide a sensible default
while still allowing callers to override it. Const generics currently support
integer, bool, and char types; floating-point and custom types are not yet
stable. Where N can be inferred from a function argument, you rarely need to
write it explicitly.

## See Also

- [const-fn](const-fn.md) - mark functions `const fn` so they can be called in
  const contexts
- [mem-assert-type-size](mem-assert-type-size.md) - assert type sizes to catch
  layout regressions
