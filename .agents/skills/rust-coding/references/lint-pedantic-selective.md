...
The `clippy::pedantic` group contains opinionated lints that aren't universally
applicable. Enabling it wholesale produces noise; selectively enabling useful
pedantic lints improves code quality without false positives.
...
1. Run `cargo clippy` on codebase
1. Discuss each warning category
1. Disable ones that don't fit your style
1. Document decisions in `clippy.toml`
