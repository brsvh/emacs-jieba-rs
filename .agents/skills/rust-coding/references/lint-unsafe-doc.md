...
The `undocumented_unsafe_blocks` lint ensures every unsafe block has a
`// SAFETY:` comment explaining why the operation is sound. Unsafe code is the
source of most memory safety bugs—documenting invariants catches mistakes and
helps reviewers.
...
1. **Why the invariants hold** - how you know they're satisfied
1. **What could go wrong** - if invariants are violated
