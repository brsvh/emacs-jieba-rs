...
A `panic!`, `.unwrap()`, or `.expect()` inside a proc-macro produces an opaque
compiler message — "proc macro panicked" — with no source location. The user
sees no indication of which part of their code triggered the error. Returning a
`syn::Error` converted to a token stream instead gives a diagnostic that points
directly at the offending span, exactly like an ordinary compiler error.
...
- Wrap proc-macro logic in a `fn(...) -> Result<TokenStream2, syn::Error>`
  helper and convert at the entry point with
  `.unwrap_or_else(|e| e.to_compile_error().into())`.
- Use `Error::new_spanned(tokens, "message")` to attach the error to a specific
  part of the input AST.
- Use `Error::new(span, "message")` when you only have a `Span`, not a token.
- Combine multiple errors with `Error::combine` rather than returning early, so
  the user sees all problems at once.
- Error messages: lowercase, no trailing punctuation (consistent with Rust
  compiler style).

## See Also

- [macro-proc-syn-quote](macro-proc-syn-quote.md) - parsing with syn, quoting
  with quote
