...
Modern CPUs predict branches to speculatively execute code. Mispredictions cause
pipeline stalls (10-20 cycles). Helping the compiler understand which branches
are likely allows it to generate optimal code layout and branch hints, improving
...
`std::hint::cold_path()` (stable since Rust 1.95) marks the enclosing code path
as unlikely so the optimizer can deprioritize it — a stable substitute for
nightly `unlikely`. Call it inside the rarely-taken branch:
...
- [opt-inline-never-cold](./opt-inline-never-cold.md) - Keeping cold code
  separate
