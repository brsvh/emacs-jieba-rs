...
`Cow` (Clone-on-Write) lets you avoid allocations when you *might* need to own
data but usually don't. It holds either a borrowed reference or an owned value,
cloning only when mutation is needed.
