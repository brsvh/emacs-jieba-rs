...
Rust's lifetime elision rules handle most common borrowing patterns
automatically. Adding explicit lifetimes where they're not needed clutters code
without adding clarity. However, understanding when elision applies helps you
know when explicit lifetimes are truly necessary.
...
Use `'_` to let the compiler infer while being explicit about the presence of a
lifetime:
...
- [own-borrow-over-clone](./own-borrow-over-clone.md) - Prefer borrowing to
