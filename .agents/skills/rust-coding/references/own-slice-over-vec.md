...
Accepting `&[T]` instead of `&Vec<T>` makes your function more flexible - it can
accept slices from arrays, vectors, or other sources. Similarly, `&str` accepts
string slices from `String`, `&'static str`, or substrings.
...
- [api-impl-asref](api-impl-asref.md) - Accept `impl AsRef<T>` for maximum
  flexibility
