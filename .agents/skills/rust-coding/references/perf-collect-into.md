...
`collect_into()` allows collecting iterator results into an existing collection,
reusing its allocation. This avoids the allocation that `collect()` would make
for a new collection.

> **Note:** `collect_into` is currently **nightly-only** (requires
> `#![feature(iter_collect_into)]`, tracking issue
> [#94780](https://github.com/rust-lang/rust/issues/94780)). On stable Rust, use
> `extend()` instead — see the Stable Alternative section below.
