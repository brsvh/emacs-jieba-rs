...
When a crate produces multiple binaries, placing them in `src/bin/` keeps the
project organized. Each file becomes a separate binary target automatically,
without manual `Cargo.toml` configuration.
...
- [proj-workspace-large](./proj-workspace-large.md) - Workspace for larger
  projects
